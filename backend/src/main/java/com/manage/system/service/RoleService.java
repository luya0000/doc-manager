...
public class RoleService {
    ...
    public List<RoleBean> getRoleList(String name, String status) throws Exception {
        // 设置模糊查询参数
        String roleName = StringUtils.isBlank(name) ? null : "%" + name.trim() + "%";
        status = status == null ? Constants.STATUE_INVALID : status;
        List<RoleDto> roleDtos = roleMapper.selectAll(roleName, status);
    ...
    public RoleBean selectByPrimaryKey(Integer userId) throws Exception {
        RoleBean bean = new RoleBean();
        RoleDto dto = roleMapper.selectByPrimaryKey(userId);
        if (dto != null) {
            BeanUtils.copyProperties(dto, bean);
        }
        return bean;
